...
namespace pf {
    ...
    void weight(vector<Particle>& particles, Mat image1, Mat image2, double (*likelihood)(int, int, Mat, Mat)){
        double sum_weight = 0.0;
        for(int i = 0; i < particles.size(); i++){
            particles[i].weight = likelihood(particles[i].x, particles[i].y, image1, image2);
            sum_weight += particles[i].weight;
        }
        for(int i = 0; i < particles.size(); i++)
            particles[i].weight /= sum_weight;
    ...
    void weight(vector<Particle>& particles, Mat image1, Mat image2, Mat image3, double (*likelihood)(int, int, Mat, Mat, Mat)){
        double sum_weight = 0.0;
        for(int i = 0; i < particles.size(); i++){
            particles[i].weight = likelihood(particles[i].x, particles[i].y, image1, image2, image3);
            sum_weight += particles[i].weight;
        }
        for(int i = 0; i < particles.size(); i++)
            particles[i].weight /= sum_weight;
    ...
    void draw_particles(vector<pf::Particle> particles, Mat image, int b, int g, int r){
        for(int i = 0; i < particles.size(); i++){
            int x = (int)particles[i].x;
            int y = (int)particles[i].y;
            circle(image, Point(x, y), 1, Scalar(b, g, r), -1);
        }
    }
}
